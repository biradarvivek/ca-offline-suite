...
const MetricIcon = ({ type }) => {
  const iconProps = { className: "h-4 w-4 text-white transition-all duration-300 sm:h-5 sm:w-5 lg:h-6 lg:w-6" };
  switch (type) {
    case 'statements': return <ClipboardList {...iconProps} />;
    case 'reports': return <FileText {...iconProps} />;
    case 'timeSaved': return <Clock {...iconProps} />;
    default: return null;
  }
};
...
const StatsMetricCard = ({ 
  ...
  return (
    <Card className="w-full overflow-hidden transition-all duration-300 border-0 rounded-xl relative backdrop-blur-xl"
          style={cardStyles}>
      ...
      <CardContent className="flex flex-col h-full p-6 gap-4 relative z-10">
        ...
        <div>
          ...
          <div className="text-sm text-white/80">
            {mainValueLabel || (type === 'timeSaved' ? 'Minutes Saved' : type === 'reports' ? 'Reports Generated' : 'Statements Processed')}
          </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              {chartType === 'line' ? (
                <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <XAxis dataKey="month" stroke="rgba(255, 255, 255, 0.3)" tick={{ fill: "rgba(255, 255, 255, 0.8)", fontSize: '12px' }} />
                  <YAxis stroke="rgba(255, 255, 255, 0.3)" tick={{ fill: "rgba(255, 255, 255, 0.8)", fontSize: '12px' }} width={30} />
                  ...
                  <Line type="monotone" dataKey="value" stroke="url(#lineGradient)" strokeWidth={3} dot={{ fill: "#fff", strokeWidth: 2 }} activeDot={{ r: 6, fill: "#fff", strokeWidth: 0 }} />
                </LineChart>
              ) : (
        ...
        <div className="grid grid-cols-2 gap-3 mt-auto">
          {bottomStats?.map((item, index) => (
            <div key={index} className="bg-white/10 p-3 rounded-xl backdrop-blur-sm hover:bg-white/15">
