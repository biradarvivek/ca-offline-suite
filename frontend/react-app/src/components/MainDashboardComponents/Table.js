...
const DataTable = ({ data = [] }) => {
 ...
 const handleCategoryChange = (transactionId, newCategory) => {
    const transaction = data.find(row => row.id === transactionId);
  ...
  const saveChanges = async () => {
    try {
      ...
      // Convert pendingChanges object to array of changes
                    ...
                    {columns.map((column) => (
                      ...
                      >
                       ...
                       {column.toLowerCase()==="category" ? (
                        ...
                        // onValueChange={(value) => handleEntityChange(row.id, value)}
                      >
                        <SelectTrigger className="w-full">
                          <SelectValue>{row[column]}</SelectValue>
                        </SelectTrigger>
                        <SelectContent>
                          {categoryOptions.map((entity) => (
